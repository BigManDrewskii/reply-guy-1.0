import{c as Q,r as l,j as e,C as ue,a as H,b as Z,u as W,Z as ie,R as ne,d as me,P as he}from"./sidepanel-DuZjw_0a.js";import{P as re}from"./icons-BVrX6sD7.js";import{C as G,a as B,b as _,c as Y,B as le,d as X}from"./badge-CmSeivz9.js";import{L as fe,B as D}from"./button-DmPrM7kr.js";import{G as ge,f as xe,c as pe,T as ee,C as be,A as ae}from"./alert-4kMVdb-k.js";import{E as ce}from"./external-link-t_hCQoZ3.js";import{s as te,e as J,g as ye,a as we}from"./openrouter-DO4vzLi3.js";import{f as oe,c as je,a as ve}from"./crm-C46ehvCP.js";/**
...
`).replace(/\\"/g,'"').replace(/\\\\/g,"\\").replace(/\\t/g,"	"):null}function Be(){const[t,s]=l.useState({}),[n,r]=l.useState({}),[i,o]=l.useState({}),[m,c]=l.useState({}),[f,g]=l.useState({}),[S,C]=l.useState("service"),[w,U]=l.useState(null),[A,b]=l.useState(0),N=W(x=>x.apiKey),R=W(x=>x.preferredModel),E=l.useRef(null),F=l.useRef(null),V=l.useRef(null),k=l.useRef(null),p=3;l.useEffect(()=>{const x=()=>{chrome.storage.local.get("voiceProfile",a=>{a.voiceProfile&&U(a.voiceProfile)})};x();const d=()=>x();return chrome.storage.onChanged.addListener(d),()=>chrome.storage.onChanged.removeListener(d)},[]);const I=l.useCallback(()=>{k.current&&(k.current.abort(),k.current=null)},[]),j=l.useCallback((x,d)=>{if(!(w!=null&&w.metrics)){g(a=>({...a,[d]:null}));return}try{const a=pe(x,w.metrics);g(y=>({...y,[d]:a}))}catch(a){console.warn("[useMessageGeneration] Voice match scoring failed:",a),g(y=>({...y,[d]:null}))}},[w]),L=l.useCallback(async(x,d,a)=>{if(!N)return;E.current=x,F.current=d,V.current=a,I();const y=new AbortController;k.current=y,o(M=>({...M,[a]:!0})),r(M=>({...M,[a]:""})),g(M=>({...M,[a]:null}));try{const M=W.getState().messageLength,z=Ve(x,d,a,w||void 0,M);let P="";await te([{role:"system",content:"You are an expert outreach copywriter. You write personalized messages that sound authentically human — never generic, never AI-sounding. You follow the Hook→Bridge→Value→CTA structure precisely."},{role:"user",content:z}],N,{signal:y.signal,onChunk:$=>{P+=$;const v=K(P);v&&r(u=>({...u,[a]:v}));try{const u=J(P),h=JSON.parse(u);h.message&&h.wordCount&&(s(T=>({...T,[a]:h})),r(T=>({...T,[a]:""})),j(h.message,a))}catch{}},onComplete:$=>{try{const v=J($),u=JSON.parse(v);s(h=>({...h,[a]:u})),r(h=>({...h,[a]:""})),o(h=>({...h,[a]:!1})),b(0),j(u.message,a)}catch{const v=K($);if(v){const u={message:v,wordCount:v.split(/\s+/).filter(Boolean).length,hook:"",voiceScore:50};s(h=>({...h,[a]:u})),j(v,a)}r(u=>({...u,[a]:""})),o(u=>({...u,[a]:!1}))}k.current=null},onError:$=>{if($ instanceof DOMException&&$.name==="AbortError"){o(h=>({...h,[a]:!1})),r(h=>({...h,[a]:""}));return}const v=$ instanceof Error?$.message:"Generation failed";if((v.includes("network")||v.includes("timeout")||v.includes("503")||v.includes("502"))&&A<p){b(T=>T+1);const h=Math.pow(2,A)*1e3;setTimeout(()=>{E.current&&F.current&&V.current&&L(E.current,F.current,V.current)},h)}else o(h=>({...h,[a]:!1})),r(h=>({...h,[a]:""})),b(0);k.current=null}},R)}catch(M){if(M instanceof DOMException&&M.name==="AbortError"){o(z=>({...z,[a]:!1})),r(z=>({...z,[a]:""}));return}o(z=>({...z,[a]:!1})),r(z=>({...z,[a]:""})),k.current=null}},[N,w,A,I,j,R]),q=l.useCallback(async(x,d,a)=>{if(!N||!(w!=null&&w.metrics))return;const y=t[a],M=f[a];if(!y||!M)return;I();const z=new AbortController;k.current=z,c(P=>({...P,[a]:!0})),r(P=>({...P,[a]:""}));try{const P=Ge(y.message,w,M.score,M.breakdown);let $="";await te([{role:"system",content:"You are a voice-matching editor. You rewrite messages to better match a specific writing style while preserving the original intent and context."},{role:"user",content:P}],N,{signal:z.signal,onChunk:v=>{$+=v;const u=K($);u&&r(h=>({...h,[a]:u}))},onComplete:v=>{try{const u=J(v),h=JSON.parse(u);s(T=>({...T,[a]:h})),r(T=>({...T,[a]:""})),c(T=>({...T,[a]:!1})),j(h.message,a)}catch{const u=K(v);if(u){const h={message:u,wordCount:u.split(/\s+/).filter(Boolean).length,hook:y.hook,voiceScore:70};s(T=>({...T,[a]:h})),j(u,a)}r(h=>({...h,[a]:""})),c(h=>({...h,[a]:!1}))}k.current=null},onError:v=>{if(v instanceof DOMException&&v.name==="AbortError"){c(u=>({...u,[a]:!1})),r(u=>({...u,[a]:""}));return}c(u=>({...u,[a]:!1})),r(u=>({...u,[a]:""})),k.current=null}},R)}catch{c($=>({...$,[a]:!1})),r($=>({...$,[a]:""})),k.current=null}},[N,w,t,f,I,j,R]),O=l.useCallback(async(x,d,a)=>{s(y=>({...y,[a]:null})),r(y=>({...y,[a]:""})),g(y=>({...y,[a]:null})),await L(x,d,a)},[L]);return{messages:t,streamingText:n,isGenerating:i,isRefining:m,voiceMatchScores:f,selectedAngle:S,setSelectedAngle:C,generateMessage:L,regenerateMessage:O,refineMessage:q,cancelGeneration:I,setMessages:s}}const Ue=["i hope this message finds you well","i wanted to reach out","i came across your","i was impressed by","i'd love to connect","i'd love to chat","i'd love to learn more","let me know if you'd be open","would you be open to","looking forward to connecting","looking forward to hearing","i believe there could be","i think there's a great opportunity","synergy","leverage","circle back","touch base","deep dive","game-changer","cutting-edge","innovative solution","value proposition","thought leader","paradigm shift","best-in-class","at the end of the day","in today's fast-paced","in the ever-evolving","it's worth noting","it goes without saying","needless to say","that being said","having said that","delve into","delve deeper","navigate the complexities","foster meaningful","cultivate","spearhead","revolutionize","streamline"],qe=["i noticed that","i was particularly","i'm reaching out because","i'm writing to","as someone who","as a fellow","given your expertise","given your background","with your experience","based on your"];function He(t){if(t.length<50)return 50;const s=t.toLowerCase().split(/\s+/),r=new Set(s).size/s.length,i=new Set;let o=0;for(let g=0;g<s.length-1;g++)i.add(`${s[g]} ${s[g+1]}`),o++;const m=o>0?i.size/o:1,c=Math.max(0,Math.min(100,(.7-r)*200)),f=Math.max(0,Math.min(100,(.9-m)*300));return(c+f)/2}function We(t){const s=t.toLowerCase();let n=0;for(const r of Ue)s.includes(r)&&n++;for(const r of qe)s.includes(r)&&n++;return Math.min(100,n*25)}function Je(t){const s=t.split(/[.!?]+/).filter(c=>c.trim().length>0);if(s.length<3)return 30;const n=s.map(c=>c.trim().split(/\s+/).length),r=n.reduce((c,f)=>c+f,0)/n.length,i=n.reduce((c,f)=>c+Math.pow(f-r,2),0)/n.length,o=Math.sqrt(i),m=r>0?o/r:0;return Math.max(0,Math.min(100,(.5-m)*200))}function _e(t){const s=t.toLowerCase(),n=["perhaps","maybe","possibly","potentially","arguably","it seems","it appears","it's possible","one might","could potentially","might consider","worth exploring","i think","i believe","in my opinion","from my perspective"],r=s.split(/\s+/).length;let i=0;for(const m of n){const c=new RegExp(m,"g"),f=s.match(c);f&&(i+=f.length)}const o=i/r*100;return Math.min(100,o*30)}function Ye(t){if(!t||t.length<20)return{score:0,label:"Too short to analyze",breakdown:{compression:0,phrases:0,structure:0,hedging:0},suggestions:[]};const s=He(t),n=We(t),r=Je(t),i=_e(t),o=Math.round(s*.2+n*.4+r*.2+i*.2),m=Math.max(0,Math.min(100,o));let c;m<=20?c="Very human":m<=40?c="Mostly human":m<=60?c="Mixed":m<=80?c="Somewhat AI":c="Very AI";const f=[];return n>40&&f.push(`Remove generic AI phrases like "I hope this finds you well" or "I'd love to connect"`),r>50&&f.push("Vary your sentence lengths — mix short punchy sentences with longer ones"),i>40&&f.push('Reduce hedging words like "perhaps", "potentially", "might consider"'),s>50&&f.push("Use more specific, unique vocabulary instead of common generic terms"),{score:m,label:c,breakdown:{compression:s,phrases:n,structure:r,hedging:i},suggestions:f}}function Ke(t){return t>=80?{label:"Sounds like you",variant:"success"}:t>=60?{label:"Close match",variant:"info"}:t>=40?{label:"Moderate match",variant:"warning"}:{label:"Generic tone",variant:"error"}}function Xe(t){return t<=20?{label:"Very human",color:"text-success"}:t<=40?{label:"Human",color:"text-info"}:t<=60?{label:"Mixed",color:"text-warning"}:{label:"AI-sounding",color:"text-destructive"}}const Ze={sentenceLength:{label:"Rhythm",desc:"Sentence length pattern"},formality:{label:"Formality",desc:"Register & tone level"},contractions:{label:"Contractions",desc:"Contraction usage"},readability:{label:"Readability",desc:"Reading level match"},pronouns:{label:"Pronouns",desc:"I/you/we usage"},punctuation:{label:"Punctuation",desc:"Marks & emphasis"}};function Qe({pageData:t,analysis:s,selectedAngle:n,onSelectAngle:r,onCopy:i,onRegenerate:o,onScheduleFollowUp:m}){const{messages:c,streamingText:f,isGenerating:g,isRefining:S,voiceMatchScores:C,generateMessage:w,regenerateMessage:U,refineMessage:A,setMessages:b}=Be(),[N,R]=l.useState(!1),[E,F]=l.useState(!1),[V,k]=l.useState(!1),p=c[n],I=f[n]||"",j=g[n],L=S[n],q=(j||L)&&I.length>0,O=C[n],x=p?Ye(p.message):null,d=x?Xe(x.score):null,a=(O==null?void 0:O.score)??(p==null?void 0:p.voiceScore)??0,y=p?Ke(a):null,M=O&&O.score<70&&!L&&!j,z=u=>{r(u),!c[u]&&!g[u]&&w(t,s,u)},P=()=>{U(t,s,n),F(!1),k(!1),o&&o()},$=()=>{s&&(A(t,s,n),k(!1))},v=()=>{i&&p&&i(p.message)};return!p&&!j&&s&&w(t,s,n),e.jsxs(G,{variant:"default",children:[e.jsx(_,{children:e.jsx(Y,{children:"Message"})}),e.jsxs(B,{className:"space-y-3",children:[(s==null?void 0:s.outreachAngles)&&e.jsx(ee.Root,{defaultValue:n,value:n,onValueChange:z,children:e.jsx(ee.List,{children:s.outreachAngles.map(u=>e.jsx(ee.Trigger,{value:u.angle,children:u.angle.charAt(0).toUpperCase()+u.angle.slice(1)},u.angle))})}),j&&!p&&!q&&e.jsxs("div",{role:"status","aria-live":"polite","aria-label":"Generating message",className:"space-y-2.5 py-3",children:[e.jsx("div",{className:"h-2.5 bg-muted rounded-full w-full animate-pulse"}),e.jsx("div",{className:"h-2.5 bg-muted rounded-full w-5/6 animate-pulse"}),e.jsx("div",{className:"h-2.5 bg-muted rounded-full w-4/6 animate-pulse"}),e.jsx("div",{className:"h-2.5 bg-muted rounded-full w-full animate-pulse"}),e.jsx("div",{className:"h-2.5 bg-muted rounded-full w-3/5 animate-pulse"})]}),q&&!p&&e.jsxs("div",{className:"rounded-lg bg-background/50 p-3",children:[e.jsxs("p",{className:"text-[13px] text-foreground whitespace-pre-wrap leading-relaxed",children:[I,e.jsx("span",{className:"inline-block w-[2px] h-[14px] bg-primary ml-0.5 align-middle animate-blink"})]}),e.jsxs("div",{className:"flex items-center gap-2 mt-2 pt-2 border-t border-border/30",children:[e.jsx("div",{className:"w-1.5 h-1.5 rounded-full bg-primary animate-pulse"}),e.jsxs("span",{className:"text-[10px] text-muted-foreground",children:[L?"Refining voice":"Generating"," · ",I.split(/\s+/).filter(Boolean).length,"w"]})]})]}),L&&q&&p&&e.jsxs("div",{className:"rounded-lg bg-background/50 p-3 border border-primary/20",children:[e.jsxs("div",{className:"flex items-center gap-1.5 mb-2",children:[e.jsx("div",{className:"w-1.5 h-1.5 rounded-full bg-primary animate-pulse"}),e.jsx("span",{className:"text-[10px] text-primary font-medium",children:"Refining voice match..."})]}),e.jsxs("p",{className:"text-[13px] text-foreground whitespace-pre-wrap leading-relaxed",children:[I,e.jsx("span",{className:"inline-block w-[2px] h-[14px] bg-primary ml-0.5 align-middle animate-blink"})]})]}),p&&!L&&e.jsxs(e.Fragment,{children:[e.jsx("div",{className:"rounded-lg bg-background/50 p-3",children:e.jsx("p",{className:"text-[13px] text-foreground whitespace-pre-wrap leading-relaxed",children:p.message})}),e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsx("div",{className:"flex items-center gap-2",children:y&&e.jsxs("button",{onClick:()=>k(!V),className:"flex items-center gap-1 group",children:[e.jsx(le,{variant:y.variant,size:"sm",children:y.label}),e.jsxs("span",{className:"text-[10px] text-muted-foreground tabular-nums",children:[a,"%"]}),e.jsx(be,{size:10,className:`text-muted-foreground/50 transition-transform ${V?"rotate-180":""}`})]})}),e.jsxs("div",{className:"flex items-center gap-2",children:[x&&d&&e.jsxs("button",{onClick:()=>F(!E),className:`flex items-center gap-1 text-[10px] ${d.color} hover:opacity-80 transition-opacity`,title:"AI-ness score — click for details",children:[e.jsx(ie,{size:10}),e.jsxs("span",{className:"tabular-nums",children:[x.score,"% AI"]})]}),e.jsx("span",{className:"text-[10px] text-muted-foreground/40",children:"·"}),e.jsxs("span",{className:"text-[10px] text-muted-foreground tabular-nums",children:[p.wordCount,"w"]})]})]}),V&&O&&e.jsxs("div",{className:"rounded-lg bg-background/50 border border-border/40 p-3 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200",children:[e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsx("span",{className:"text-[11px] font-medium text-foreground",children:"Voice Match Breakdown"}),e.jsxs("span",{className:`text-[11px] font-medium ${y?`text-${y.variant}`:"text-muted-foreground"}`,children:[O.score,"/100"]})]}),e.jsx("div",{className:"space-y-1.5",children:Object.entries(O.breakdown).map(([u,h])=>{const T=Ze[u];return e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx("span",{className:"text-[10px] text-muted-foreground w-[70px] shrink-0",title:T.desc,children:T.label}),e.jsx("div",{className:"flex-1 h-1.5 bg-muted rounded-full overflow-hidden",children:e.jsx("div",{className:`h-full rounded-full transition-all duration-500 ${h>=70?"bg-success":h>=40?"bg-warning":"bg-destructive"}`,style:{width:`${Math.max(3,h)}%`}})}),e.jsx("span",{className:"text-[9px] text-muted-foreground/60 w-6 text-right tabular-nums",children:h})]},u)})}),M&&e.jsxs("div",{className:"pt-2 border-t border-border/30",children:[e.jsxs(D,{variant:"outline",size:"sm",onClick:$,className:"w-full text-[11px]",children:[e.jsx(ne,{size:11}),"Refine Voice Match (",O.score,"% → 85%+)"]}),e.jsx("p",{className:"text-[9px] text-muted-foreground/50 text-center mt-1",children:"Uses a second LLM pass to improve weak dimensions"})]})]}),E&&x&&e.jsxs("div",{className:"rounded-lg bg-background/50 border border-border/40 p-3 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200",children:[e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsx("span",{className:"text-[11px] font-medium text-foreground",children:"AI Detection Breakdown"}),e.jsx("span",{className:`text-[11px] font-medium ${d.color}`,children:x.label})]}),e.jsx("div",{className:"space-y-1.5",children:[{label:"Phrases",value:x.breakdown.phrases,desc:"Common AI phrases"},{label:"Structure",value:x.breakdown.structure,desc:"Sentence uniformity"},{label:"Hedging",value:x.breakdown.hedging,desc:"Qualifiers & hedges"},{label:"Vocab",value:x.breakdown.compression,desc:"Word diversity"}].map(({label:u,value:h,desc:T})=>e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx("span",{className:"text-[10px] text-muted-foreground w-14 shrink-0",title:T,children:u}),e.jsx("div",{className:"flex-1 h-1.5 bg-muted rounded-full overflow-hidden",children:e.jsx("div",{className:`h-full rounded-full transition-all duration-300 ${h<=30?"bg-success":h<=60?"bg-warning":"bg-destructive"}`,style:{width:`${Math.max(2,h)}%`}})}),e.jsx("span",{className:"text-[9px] text-muted-foreground/60 w-6 text-right tabular-nums",children:h})]},u))}),x.suggestions.length>0&&e.jsxs("div",{className:"pt-2 border-t border-border/30",children:[e.jsx("p",{className:"text-[10px] font-medium text-muted-foreground mb-1",children:"Suggestions"}),e.jsx("ul",{className:"space-y-0.5",children:x.suggestions.map((u,h)=>e.jsxs("li",{className:"text-[10px] text-muted-foreground/70 flex gap-1.5",children:[e.jsx("span",{className:"shrink-0",children:"•"}),e.jsx("span",{children:u})]},h))})]})]}),p.hook&&e.jsxs("p",{className:"text-[11px] text-muted-foreground/70 italic leading-relaxed",children:["Hook: ",p.hook]}),e.jsx(Te,{text:p.message,onCopy:v}),e.jsxs("div",{className:"flex gap-2",children:[e.jsxs(D,{variant:"ghost",size:"sm",onClick:P,disabled:j||L,className:"flex-1",children:[e.jsx(ne,{size:13,className:j?"animate-spin":""}),"Regenerate"]}),e.jsxs(D,{variant:"ghost",size:"sm",onClick:()=>R(!0),className:"flex-1","aria-label":"Edit this message",children:[e.jsx(Se,{size:13}),"Edit"]})]}),m&&e.jsxs(D,{variant:"ghost",size:"sm",onClick:m,className:"w-full text-muted-foreground",children:[e.jsx(de,{size:13}),"Schedule Follow-ups"]})]})]}),N&&p&&e.jsx(Ie,{initialMessage:p.message,onSave:u=>{b(h=>({...h,[n]:{...p,message:u,wordCount:u.split(/\s+/).filter(Boolean).length}})),R(!1)},onClose:()=>R(!1)})]})}const et=l.memo(Qe,(t,s)=>{var n,r,i,o,m,c,f,g;return t.pageData.url===s.pageData.url&&t.selectedAngle===s.selectedAngle&&((n=t.analysis)==null?void 0:n.personName)===((r=s.analysis)==null?void 0:r.personName)&&((i=t.analysis)==null?void 0:i.confidence)===((o=s.analysis)==null?void 0:o.confidence)&&((c=(m=t.analysis)==null?void 0:m.outreachAngles)==null?void 0:c.length)===((g=(f=s.analysis)==null?void 0:f.outreachAngles)==null?void 0:g.length)});function tt(t,s){return s?t==="x"?`https://x.com/messages/${s}`:t==="linkedin"?"https://www.linkedin.com/messaging/compose/":null:null}function st({isOpen:t,onClose:s,onLogged:n,platform:r,username:i}){const[o,m]=l.useState(!1);l.useEffect(()=>{t&&m(!0)},[t]);const c=l.useCallback(w=>{t&&(w.key==="Escape"?S():w.key==="Enter"&&g())},[t]);if(l.useEffect(()=>(document.addEventListener("keydown",c),()=>document.removeEventListener("keydown",c)),[c]),!t)return null;const f=tt(r,i),g=()=>{n(),m(!1),setTimeout(()=>s(),200)},S=()=>{m(!1),setTimeout(()=>s(),200)},C=()=>{f&&window.open(f,"_blank"),g()};return e.jsx("div",{className:`
...
Tone: Respectful, brief, confident. Like closing a door gently.`}]}}async function ot(t){for(const s of t.messages){const n=`followup-${t.contactId}-${s.sequence}`,r=Date.now()+s.delayMs;try{await chrome.alarms.create(n,{when:r});const i=`followup_${t.contactId}_${s.sequence}`;await chrome.storage.local.set({[i]:{contactId:t.contactId,contactName:t.contactName,sequence:s.sequence,tone:s.tone,prompt:s.prompt,scheduledFor:r}}),await X.touchpoints.add({contactId:t.contactId,type:"follow_up_scheduled",angle:t.originalAngle,message:`Follow-up #${s.sequence} (${s.tone}) scheduled for ${s.delay}`,platform:"system",timestamp:Date.now()})}catch(i){console.warn(`[follow-up] Failed to schedule alarm ${n}:`,i)}}}const it=[{value:"short",label:"Short",desc:"40-80w"},{value:"medium",label:"Medium",desc:"100-150w"},{value:"long",label:"Long",desc:"180-250w"}];function xt({initialData:t}){const[s,n]=l.useState(t),[r,i]=l.useState(!t),[o,m]=l.useState("service"),[c,f]=l.useState(!1),[g,S]=l.useState(!1),[C,w]=l.useState(""),[U,A]=l.useState(null),{analysis:b,isAnalyzing:N,isDebouncing:R,debounceCountdown:E,error:F,analyzePage:V,cancelAnalysis:k}=rt(),p=W(d=>d.messageLength),I=W(d=>d.setMessageLength),{add:j}=me();l.useEffect(()=>{const d=a=>{if(a.currentPageData){const y=a.currentPageData.newValue;n(y),i(!1),S(!1),A(null)}};return chrome.storage.session.onChanged.addListener(d),()=>chrome.storage.session.onChanged.removeListener(d)},[]),l.useEffect(()=>{(async()=>{if(s!=null&&s.url)try{const a=await je(s.url);if(a&&a.totalMessages>0){A(`You've already sent ${a.totalMessages} message${a.totalMessages>1?"s":""} to this contact.`);return}const y=await X.conversations.where("pageUrl").equals(s.url).count();y>0?A(`You've already sent ${y} message${y>1?"s":""} to this page.`):A(null)}catch{}})()},[s==null?void 0:s.url]);const L=()=>{s&&!b&&!N&&!g&&(S(!0),V(s))},q=d=>{d&&w(d),setTimeout(()=>f(!0),100)},O=async()=>{if(!(!s||!C))try{await X.conversations.add({id:`${Date.now()}-${Math.random().toString(36).slice(2,9)}`,platform:s.platform,pageUrl:s.url,pageName:(b==null?void 0:b.personName)||s.name||s.ogTitle||s.hostname,sentMessage:C,angle:o,sentAt:Date.now(),status:"sent"});try{const d=await oe(s,b||void 0);await ve({contactId:d,type:"copied",angle:o,message:C,platform:s.platform,timestamp:Date.now(),messageLength:p})}catch(d){console.warn("[OutreachScreen] Failed to update contact:",d)}}catch(d){console.error("[OutreachScreen] Failed to log conversation:",d)}};if(r)return e.jsxs("div",{className:"space-y-3",children:[e.jsx(he,{}),e.jsxs("div",{className:"space-y-2",children:[e.jsx("div",{className:"h-3 bg-muted rounded w-1/4"}),e.jsx("div",{className:"h-3 bg-muted rounded w-full"}),e.jsx("div",{className:"h-3 bg-muted rounded w-5/6"})]})]});if(!s)return e.jsx("div",{className:"text-center py-8",children:e.jsx("p",{className:"text-[13px] leading-relaxed text-muted-foreground",children:"Waiting for page data..."})});const x=s.isProfile||s.name;return e.jsxs("div",{className:"space-y-3",children:[x?e.jsx(Re,{data:s}):e.jsx(Me,{data:s}),s.isThread&&s.threadContext&&s.threadContext.length>0&&e.jsx(G,{variant:"default",children:e.jsxs(B,{className:"p-3",children:[e.jsxs("div",{className:"flex items-center gap-2 mb-1.5",children:[e.jsx(ke,{size:13,className:"text-info"}),e.jsxs("span",{className:"text-[11px] font-medium text-foreground",children:["Thread detected · ",s.threadContext.length," messages"]})]}),e.jsx("p",{className:"text-[11px] text-muted-foreground leading-relaxed",children:"Thread context will be used to generate more relevant messages."})]})}),U&&e.jsx(ae,{variant:"warning",children:e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx(de,{size:14,className:"shrink-0"}),e.jsx("span",{children:U})]})}),F&&e.jsx(ae,{variant:"error",children:F}),s&&!b&&!N&&!R&&!g&&e.jsx(G,{variant:"default",children:e.jsxs(B,{className:"p-5 text-center",children:[e.jsx("div",{className:"w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-3",children:e.jsx(ie,{size:18,className:"text-primary"})}),e.jsx("p",{className:"text-sm font-medium text-foreground mb-1",children:"Ready to analyze"}),e.jsxs("p",{className:"text-xs text-muted-foreground mb-4",children:["Generate personalized outreach for"," ",e.jsx("span",{className:"text-foreground font-medium",children:s.name||s.ogTitle||s.hostname})]}),e.jsx(D,{onClick:L,variant:"primary",size:"md",className:"w-full",children:"Analyze This Page"})]})}),R&&e.jsx(G,{variant:"default",children:e.jsxs(B,{className:"p-4 text-center",children:[e.jsxs("div",{className:"flex items-center justify-center gap-2 mb-2",children:[e.jsx("div",{className:"w-2 h-2 rounded-full bg-info animate-pulse"}),e.jsx("span",{className:"text-xs font-medium text-foreground",children:"Starting analysis..."})]}),e.jsxs("p",{className:"text-[11px] text-muted-foreground",children:["Analyzing in ",E,"s"]}),e.jsx(D,{onClick:k,variant:"ghost",size:"xs",className:"mt-2",children:"Cancel"})]})}),N&&!b&&e.jsx(G,{variant:"default",children:e.jsxs(B,{className:"p-4 text-center",children:[e.jsxs("div",{className:"flex items-center justify-center gap-2",children:[e.jsx("div",{className:"w-1.5 h-1.5 rounded-full bg-info animate-pulse"}),e.jsx("span",{className:"text-xs font-medium text-foreground",children:"Analyzing page..."})]}),e.jsxs("div",{className:"mt-3 space-y-2",children:[e.jsx("div",{className:"h-2 bg-muted rounded-full w-full animate-pulse"}),e.jsx("div",{className:"h-2 bg-muted rounded-full w-3/4 animate-pulse"})]})]})}),b&&e.jsxs(e.Fragment,{children:[e.jsxs(G,{variant:"default",children:[e.jsx(_,{children:e.jsx(Y,{children:"Confidence"})}),e.jsxs(B,{children:[e.jsx(nt,{value:b.confidence,showValue:!0,size:"md"}),b.confidenceReason&&e.jsx("p",{className:"text-[11px] leading-relaxed text-muted-foreground mt-2",children:b.confidenceReason})]})]}),b.summary&&e.jsxs(G,{variant:"default",children:[e.jsx(_,{children:e.jsx(Y,{children:"Summary"})}),e.jsx(B,{children:e.jsx("p",{className:"text-[13px] leading-relaxed text-muted-foreground",children:b.summary})})]}),b.interests&&b.interests.length>0&&e.jsxs(G,{variant:"default",children:[e.jsx(_,{children:e.jsx(Y,{children:"Interests"})}),e.jsx(B,{children:e.jsx("div",{className:"flex flex-wrap gap-1.5",children:b.interests.map((d,a)=>e.jsx(le,{variant:"outline",size:"sm",children:d},a))})})]}),e.jsxs(G,{variant:"default",children:[e.jsx(_,{children:e.jsx(Y,{children:"Message Length"})}),e.jsx(B,{children:e.jsx("div",{className:"flex rounded-lg bg-muted/50 p-0.5 gap-0.5",children:it.map(d=>e.jsxs("button",{onClick:()=>I(d.value),className:`flex-1 py-1.5 px-2 rounded-md text-center transition-all duration-200 ${p===d.value?"bg-background text-foreground shadow-sm":"text-muted-foreground hover:text-foreground"}`,children:[e.jsx("span",{className:"text-[11px] font-medium block",children:d.label}),e.jsx("span",{className:"text-[9px] opacity-60 block",children:d.desc})]},d.value))})})]})]}),b&&s&&e.jsx(et,{pageData:s,analysis:b,selectedAngle:o,onSelectAngle:m,onCopy:q,onScheduleFollowUp:async()=>{if(!s||!C){j({title:"Copy a message first",variant:"error"});return}try{const d=await oe(s,b||void 0),a=await X.contacts.get(d);if(!a)return;const y=at(a,C,o,s.platform);await ot(y),j({title:"Follow-ups scheduled",description:"3 reminders: 3d, 7d, 14d",variant:"success"})}catch(d){console.error("[OutreachScreen] Failed to schedule follow-ups:",d),j({title:"Failed to schedule follow-ups",variant:"error"})}}}),e.jsx(st,{isOpen:c,onClose:()=>f(!1),onLogged:O,platform:s.platform,username:s.username})]})}export{xt as default};
